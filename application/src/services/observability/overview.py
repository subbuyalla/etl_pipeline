...
def build_overview_kpis(conn, rng: dict, **filters) -> list[dict]:
    ...
    return [
        make_kpi(
            ...
            value=pipe_n,
            display=str(pipe_n),
            subtext=f"{pipe_n} active pipelines",
            description="Total number of active data pipelines.",
            formula="Active Pipelines",
            calculation_note=f"{pipe_n} data pipelines currently configured and monitored.",
        ),
        make_kpi(
            id="success_rate",
